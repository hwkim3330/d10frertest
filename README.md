...
| 지표 | FRER (Nov 3) | Non-FRER (Nov 4) | 차이 |
|------|------|----------|---------|
| **평균 레이턴시** | 0.356 ms | 0.401 ms | ⚠️ **혼재** (일부 개선, 일부 악화) |
| **P99 레이턴시** | 0.676 ms | 0.814 ms | ⚠️ **혼재** (일관성 부족) |
| **Jitter** | 0.101 ms | 0.128 ms | ⚠️ **혼재** (128B, 1518B는 Non-FRER 우수) |
| **소형 프레임 처리량** | 64, 256, 512B 우수 | 128B 우수 | ⚠️ **일부만 개선** |
| **대형 프레임 처리량** | - | 1280B 우수 | ⚠️ **-6.1%** (1280B) |
| **R-TAG 오버헤드** | 4 bytes | 없음 | 64B: 6.25%, 1280B: 0.31% |

> ⚠️ **주의**: 테스트 날짜가 다르고 결과가 일관성이 부족합니다. FRER의 주요 이점은 "신뢰성"(이중 경로 자동 전환)이지 "성능"이 아닙니다.
>
