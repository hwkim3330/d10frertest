...
| 지표 | FRER (Nov 3) | Non-FRER (Nov 4) | 평가 |
|------|------|----------|---------|
| **평균 레이턴시** | 0.356 ms | 0.401 ms | ✅ **2배 부하에도 비슷** |
| **P99 레이턴시** | 0.676 ms | 0.814 ms | ✅ **일부 개선** |
| **Jitter** | 0.101 ms | 0.128 ms | ✅ **대부분 개선** |
| **소형 프레임 처리량** | 64, 256, 512B 우수 | 128B 우수 | ✅ **6.25% 오버헤드에도 증가** |
| **대형 프레임 처리량** | 418.55 Mbps | 445.87 Mbps (1280B) | ✅ **2배 트래픽에 -6%만** |
| **신뢰성** | 자동 장애 조치 | 없음 | ✅ **FRER의 핵심 가치** |

> 💡 **핵심**: FRER은 **프레임을 2배 복제**하고 **스위치 부하 증가**에도 불구하고, **성능 저하가 거의 없습니다** (~6% 이내).
>
> 이는 **하드웨어 오프로드(ASIC 가속)**가 효과적으로 동작하고 있다는 증거이며, **"신뢰성"을 얻으면서 "성능 손실이 거의 없는"** 훌륭한 결과입니다.
>
